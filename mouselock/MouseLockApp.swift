...
final class AppDelegate: NSObject, NSApplicationDelegate, ObservableObject {
    ...
    private let pullbackDistance: CGFloat = 80.0
    
    // 安全区域：鼠标回到这个区域内才重置 warp 状态
    private let safeZone: CGFloat = 50.0
    ...
    private func checkAndConstrainMouse() {
        ...
        if isOutOfBounds && !hasWarped {
            ...
            if cgPoint.x < cachedMinX {
                constrainedX = cachedMinX + pullbackDistance
            } else if cgPoint.x > cachedMaxX {
                constrainedX = cachedMaxX - pullbackDistance
            }
            
            // Y 轴约束：如果超出边界，拉回到安全距离内
            if cgPoint.y < cachedMinY {
                constrainedY = cachedMinY + pullbackDistance
            } else if cgPoint.y > cachedMaxY {
                constrainedY = cachedMaxY - pullbackDistance
            }
